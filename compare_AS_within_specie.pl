...
sub compare_AS_within_wheat
{
	...
	{
		next unless (scalar keys %{$gene_pos{$gene}}) > 1;
...
sub compare_iso
{
	...
	my %as_types = compare_introns(@introns, \@exon_vecs, \@array);
	return \%as_types;
}

sub compare_introns
{
	my $exon_array_ref = pop;
	my $exon_vecs = pop;
	my @introns = @_;
...
sub compare_two_iso_introns
{
	...
	{ 
		...
		unless($index==0 or $index==(scalar @$ina -1)){push @{$types{$id}}, 'IntronR' if @overlap == 0};
		my $covered_by_exon = 0;
		foreach ($s_a, $e_a)
		{
			$covered_by_exon ++ if vec($exon_vec_b, $_, 1) == 1;
		}
		push @{$types{$id}}, 'IntronR' if $covered_by_exon ==(abs($s_a-$e_a)+1);
		$index++;
	}
	return %types;
}
