...
sub run_chisq_test
{
	...
	print PO join("\t", qw(cluster_id pvalue_A pvalue_B pvalue_D count_A count_B count_D SIL_pvalue_A SIL_pvalue_B SIL_pvalue_D A_effect B_effect D_effect)), "\n";
	foreach my $cluster (keys %$hashref)
	{
		my @pvalues = qw(NA NA NA);
		my @silence_pvalues = qw(NA NA NA);
		my @binom_pvalues = qw(NA NA NA);
		my @cnt = qw(NA NA NA);
		...
		foreach my $sub (@sub_genome)
		{
			...
			my $binom_pvalue = binomial_test(@allele_cnt, 0.5);
			my $silence_p = binomial_test(@allele_cnt, 0.05);
			$pvalue *= -1 if $allele_cnt[0]<($total/3) and $pvalue < 0.05;
			$pvalues[$sub] = $pvalue;
			$binom_pvalues[$sub] = 2*(1-$binom_pvalue);
			$silence_pvalues[$sub] = $silence_p;
			# test AB:D = 1:1
			if ($sub == 2)
			{
				$chisq_test->load_data([[@allele_cnt], [int($total/2), int($total/2)]]);
				$test_d_pvalue = $chisq_test->p_value;
				$test_d_pvalue *=-1 if $allele_cnt[0]<($total/2) and $test_d_pvalue < 0.05;
			}
		}
		my @effect = check_effect(@pvalues, @cnt);
		check_silence(\@effect, \@silence_pvalues);
		print PO join("\t", ($cluster, @pvalues, @cnt, @silence_pvalues, @effect)), "\n";
...
sub check_silence
{
	...
	foreach (0..2)
	{
		next unless $effect->[$_] =~ /SILEN/;
		$effect->[$_] =~ s/SILENCE/DOWN/ unless $p->[$_]>0.01;
	}
}
...
sub check_effect
{
	...
	foreach my $ind (0..2)
	{
		...
		else
		{
			...
			{
				my @cnt = split /\//, $arr[$ind+3];
				#print STDERR '$arr[$ind+3]: ', $arr[$ind+3], "\n";
				$return[$ind] = $gn[$ind] . ($cnt[0]>($cnt[1]/2)?"_UP":"_DOWN");
				$return[$ind] = $gn[$ind] . "_SILENCE" if $cnt[0] < ($cnt[1]*0.1)
			}
			else
			{
...
sub count_allele_freq
{
	...
	while(<F>)
	{
		...
		foreach (0 .. $#alleles)
		{
			...
			$count_allele_each_cluster{$cluster}{$sub_genome}[$_] += $add;
