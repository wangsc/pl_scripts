...
sub output
{
	...
	my @record;
	my @names = qw(ALternative_donor_exon Alternative_acceptor_exon Retained_intron spliced_intron Skipped_exon Retained_exon Conserved_exon Coverage);
...
sub read_wise_gff
{
	...
	while (<IN>)
	{
		...
		if (/^\/\//)
		{
			...
			push @{$return_hash{$iso_id}}, [@array] if @array > 1;
			@array = ();
			next;
		}
		my @t = split /\t/, $_;
...
sub compare_structure
{
	...
	my ($wise_vec, $wise_max) = construct_vec($wise_ref, 0); # 0 for genewise 
	my ($pasa_vec, $pasa_max) = construct_vec($pasa_ref, 1); # 1 for pasa 
